...
type
  ...
  //Рахує барицентричні координати трикутника за формулою, яка припускає, що координати
  //пропорційні до площ трикутників утворенних з точкою p
  //Повертає тільки 2 координати тому, що зможемо відновити третю: w = 1 - (u + v)
...
begin
  ...
  begin
       ...
       //Перевіряємо чи точка в межах трикутника
       if(barycentricP.x >= 0.0) and (barycentricP.y >= 0.0)
          and ((barycentricP.x + barycentricP.y) <= 1.0)  then
          begin

          //Відновлюємо третю координату
          var thirdCoord := 1.0 - (barycentricP.x + barycentricP.y);

          //Так як ця перевірка можлива тільки, якщо точка в трикутнику
          //отже сума її координат має дорівнювати 1.0
          //Тому для перевірки того чи точка знаходиться на координаті трикутника
          //ми можемо використати просто перевірку нижче
          if(barycentricP.x = 1.0) or (barycentricP.y = 1.0)
             or (thirdCoord = 1.0) then
              Writeln('Point on the triangle vertex!')
          else if(barycentricP.x = 0.0) or (barycentricP.y = 0.0) //Перша умова з попередньої превірки застосовується і тут
